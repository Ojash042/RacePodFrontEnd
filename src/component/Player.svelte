...
<div class="w-100 fixed-bottom px-5 flex-shrink-0 widget flex-row">
    <div class="flex-shrink-0 widget-info bg-secondary d-flex">
        <div class=" flex-row align-items-center m-auto w-25 d-flex">
            <img
                class="poster"
                src={JSON.stringify(play) === "{}"
            ...
            />
            <div
                class="flex-column p-auto m-auto d-flex align-items-start"
                style="font-size:small;"
            >
                <div class="row ms-1" style="font-weight: bold;">
                    {play["title"] === undefined
                        ? ""
                        : play["title"].split(" ").slice(0, 8).join(" ") +
                          (play["title"].split(" ").length > 7 ? "..." : "")}
                </div>
        ...
        {#key $queue}
        {#key soundBufferd}
            <div class="mx-5 flex-row align-items-center justify-content-between d-flex p-auto"> 
                {`${String(Math.floor(seek / 3600)).padStart(2, "0")}:${String(Math.floor(seek / 60) % 60).padStart(2, "0")}:${String(Math.floor(seek) % 60).padStart(2, "0")}`}
            </div>
            <div class="flex-column d-flex align-items-center">  
                            <div class="flex-row align-items-center d-flex">
                                <button class="btn flex-row d-flex btn-secondary fa-solid bg-transparent border-0 fa-backward" disabled={testSound() === "loading"}
                                    on:click={() => sound.seek(Math.max(seek - 15, 0))}>
                                </button>
                                <button class="btn flex-row d-flex btn-secondary fa-solid bg-transparent border-0 {playButtonState[testSound()]}"
                                    disabled={testSound() === "loading"}
                                    on:click={() => {
                                    console.log(`Seek: ${Math.floor(seek)}`)
                                ...
                                <button class="btn flex-row d-flex btn-secondary fa-solid bg-transparent border-0 fa-forward" disabled={testSound() === "loading"}
                                    on:click={() =>
                                        sound.seek(
                                            Math.min(seek + 15, duration),
                                        )}>
                                </button>
                            </div>
                            <input type="range" id="player" value={Math.floor(seek)}
                                step="15" min="0" max={duration} on:input={() => {
                                    sound.seek(document.getElementById("player").value, id,);}}/>
                                    </div>
            <div class="mx-5 flex-row align-items-center justify-content-between d-flex p-auto">
                {`${String(Math.floor(duration / 3600)).padStart(2, "0")}:${String(Math.floor(duration / 60) % 60).padStart(2, "0")}:${String(Math.floor(duration) % 60).padStart(2, "0")}`}
            </div>
            {/key}
            {/key}
            {/key}
